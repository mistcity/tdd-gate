...
export function handleStop(_sessionId, cwd, config, journal, circuitBreaker) {
    // 1. If completionAudit is disabled, allow
    if (!config.completionAudit)
        return { action: 'allow' };
    ...
    try {
        ...
        const headOutput = execFileSync('git', ['-C', cwd, 'diff', '--name-only', 'HEAD'], gitOpts);
        const cachedOutput = execFileSync('git', ['-C', cwd, 'diff', '--name-only', '--cached'], gitOpts);
        const parseLines = (output) => output.split('\n').map((f) => f.trim()).filter((f) => f.length > 0);
        changedFiles = new Set([...parseLines(headOutput), ...parseLines(cachedOutput)]);
    }
    catch (err) {
        if (err.code !== 'ENOENT') {
            process.stderr.write(`[tdd-gate] git diff failed (completion audit skipped): ${err instanceof Error ? err.message : String(err)}\n`);
        }
        return { action: 'allow' };
    }
    ...
    if (!hasDirectViolation && !hasImpactViolation)
        return { action: 'allow' };
    ...
    if (hasDirectViolation) {
        ...
        messageParts.push('Completion audit: The following implementation files were changed without corresponding test files:', ...lines, 'Please add tests before completing.');
    }
    if (hasImpactViolation) {
        if (messageParts.length > 0)
            messageParts.push('');
        messageParts.push('Impact analysis: The following files depend on your changes and need tests:', ...impactLines);
    }
