 ...
 */
export function buildImportPattern(basename, language) {
 ...
 */
export function findDependents(filePath, language, projectRoot, options) {
    ...
    try {
        const stdout = execFileSync('grep', ['-rl', ...excludeArgs, ...includeArgs, '-E', pattern, projectRoot], {
        ...
        const filtered = files.filter((f) => {
            if (path.resolve(f) === selfResolved)
                return false;
            if (langDef?.isTestFile(path.basename(f)))
                return false;
            return true;
        });
        ...
        process.stderr.write(`[tdd-gate] findDependents failed for ${path.basename(filePath)} (fail-open): ${err instanceof Error ? err.message : String(err)}\n`);
        return [];
    }
}
// ---------------------------------------------------------------------------
